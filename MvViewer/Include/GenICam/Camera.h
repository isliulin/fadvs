...
public:
	...
	enum ECameraType
	{
		...
		typeUndefined	///< \~chinese 未知类型      ///< \~english Undefined Camera
	};
	...
	/// \~chinese
	/// \brief 读用户自定义数据。相机内部保留32768字节用于用户存储自定义数据（此功能针对大华相机，其它品牌相机无此功能）。
	/// \param [out] pBuffer 数据缓冲的指针。
	/// \param [in] dwCount 期望读出的字节数
	/// \return 返回用户实际读取到的字节数
	/// \~english read user-defined data; Inside the camera, there are 32768 bytes reserved for user to store private data (Only Dahua has this function)
	...
	/// \return 返回用户实际写入的字节数
	/// \~english write user-defined data; Inside the camera, there are 32768 bytes reserved for user to store private data (Only Dahua has this function)
	/// \param [out] pBuffer pointer of the data buffer
	/// \param [in]  dwCount byte count expected
