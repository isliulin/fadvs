...
#include <math.h>
#include "TSCtrlSys.h"
#include "ImgMarkGeoSet1.h"


// CImgMarkGeoSet1

IMPLEMENT_DYNAMIC(CImgMarkGeoSet1, CStatic)

CImgMarkGeoSet1::CImgMarkGeoSet1()
{
...
void CImgMarkGeoSet1::PaintImage(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::Gr_DrawText(Graphics *gr,int x,int y,CString str,Color cl/* =Color */)
{
...
CRect CImgMarkGeoSet1::GetImgPaintRect(CRect rect)
{
...
void CImgMarkGeoSet1::ImgInit(long lImageWidth,long lImageHeight,int nPixel)
{
...
void CImgMarkGeoSet1::PaintDraw(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::PaintCross(CDC *pDC, CRect rect)
{
	CPoint cp;
...
void CImgMarkGeoSet1::PaintTriangle(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::PaintSquare(CDC *pDC, CRect rect)
{
	CPoint cp;
...
void CImgMarkGeoSet1::PaintRing(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::PaintRectangle(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::PaintRoiRect(CDC *pDC, CRect rect)
{
	CPoint tp[4];
...
void CImgMarkGeoSet1::PaintScale(CDC *pDC, CRect rect)
{
	CPoint point;
	...
	pDC->LineTo(point.x,rect.BottomRight().y);
	pDC->MoveTo(rect.TopLeft().x,point.y);
	pDC->LineTo(rect.BottomRight().x,point.y);
	for(long i=point.x-length;i>0;i=i-length)
	{
		pDC->MoveTo(i,point.y-height/2);
		pDC->LineTo(i,point.y+height/2);
		pDC->MoveTo(point.x*2-i,point.y-height/2);
		pDC->LineTo(point.x*2-i,point.y+height/2);
	}
...
void CImgMarkGeoSet1::PaintImage2(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::PaintEllipse(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::PaintDiamond(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::DrawCircle(CDC *pDC, CPoint point, int r, BOOL bFull)
{
	if(!bFull)
	{
		CGdiObject *pOldGdi = pDC->SelectStockObject(NULL_BRUSH);
...
void CImgMarkGeoSet1::DrawCircleEx(CDC *pDC, CPoint point, int r, BOOL bFull)
{
...
void CImgMarkGeoSet1::DrawSquare(CDC *pDC, CPoint point, int len, BOOL bFull/* =TRUE */)
{
...
void CImgMarkGeoSet1::PaintCircle(CDC *pDC, CRect rect)
{
	CPoint cp;
...
void CImgMarkGeoSet1::OnMouseMove(UINT nFlags, CPoint point)
{
	if(m_BtSel && !m_RoiSel)
	{
		switch(m_modtype)
...
void CImgMarkGeoSet1::OnLButtonDown(UINT nFlags, CPoint point)
{
	CPoint cp;
	//if(m_nCurStep==1)
	{
...
void CImgMarkGeoSet1::OnLButtonUp(UINT nFlags, CPoint point)
{
	m_BtSel = FALSE;
	m_RoiSel = FALSE;
	CStatic::OnLButtonUp(nFlags, point);
}
void CImgMarkGeoSet1::OnTimer(UINT_PTR nIDEvent)
{
	if(m_GatherSign)
		Invalidate();
...
void CImgMarkGeoSet1::DrawImgEx(CDC *pDC, CRect rect)
{
...
void CImgMarkGeoSet1::DrawSearch(CDC *pDC, CRect rect)
{
	int dr = 10;
	if(m_iCount>0)
	{
...
void CImgMarkGeoSet1::DrawImg(BYTE *pImg,int delay)
{
	m_bDisplay = TRUE;
	m_pScerchImg = pImg;
	m_DrawDelay = delay;
...
void CImgMarkGeoSet1::SetSearchResult(double *dScore,double *dXPos,double *dYPos,double *dAng,double *dScale,int iCount,double dTime)
{
	pScore = dScore;
	pXPos = dXPos;
	pYPos = dYPos;
...
void CImgMarkGeoSet1::ResetParam()
{
	long tmp;
